    ...
    'jquery/validate'
        ...
        return Component.extend({
            ...
            initialize: function (config) {
                ...
                this.fulfilmentSetting = ko.observable();
                this.analysisCode = ko.observableArray([
                    {value: 1, label: 'Yes'},
                    {value: 0, label: 'No'}
                ]);

                this.selectAnalysisCode = ko.observable(window.quote().analysis_code);

                this.selectAnalysisCode.subscribe(function (newValue) {
                    window.quote().analysis_code = newValue;
                }.bind(this));

                this.stockTypes = ko.observableArray([
                    {id: 16049, label: 'Stocked'},
                    {id: 16050, label: 'Drop Shipped'}
                ]);
            ...
            editQuoteItem: function(item, updateType)
            {
                ...
                $.each(this.quoteItems(), function(index, data) {
                    if(item.product_id() == data().product_id()) {
                        ...
                        var configPriceTax = self.config.taxClasses;
                        ...
                        switch(updateType) {
                            case 'custom_price':
                                ...
                                if(configPriceTax){
                                    var tax = (rowTotal * item.tax_percent()) / (100 + item.tax_percent());
                                }else{
            ...
            addQuoteItem: function(product, event, configuredOptions = false)
            {
                ...
                $.each(this.quoteItems(), function(index, data) {
                    if(product.product_id == data().product_id()) {
                        ...
                        if((!customOptions && !checkSeleteOption()) || customOptions == JSON.stringify(self.productCustomOptionsData())) {
                            ...
                            if(configPriceTax){
                                var tax = (rowTotal * product.tax_percent) / (100 + product.tax_percent);
                            }else{
                                var tax = (rowTotal / 100) * product.tax_percent;
                            }
                            data().tax(tax);
                ...
                var originalPrice = parseFloat(product.original_price).toFixed(2)
                ...
                if(self.productCustomOptionsData().length) {
                    $.each(self.productCustomOptionsData(), function(index, data) {
                        if(data.value[0]) {
                            ...
                            price = (Number(price) + Number(optionPrice)).toFixed(2)
                            custom_price = price
                        }
                    })
                }

                var discount = parseFloat(Number(price) - Number(custom_price)).toFixed(2);
                var discountPercent = parseFloat((Number(discount) / Number(price)) * 100).toFixed(2);
                if(newItem) {
                    ...
                    if(configPriceTax){
                        var tax = (price * product.tax_percent) / (100 + product.tax_percent);
                    }else{
                        var tax = (price / 100) * product.tax_percent;
                    }
                    ...
                    var item = ko.observable({
                        ...
                        custom_options: self.productCustomOptionsData().length ? ko.observable(JSON.stringify(self.productCustomOptionsData())) : false,
                        ...
                        price: ko.observable(originalPrice),
            ...
            createCustomProduct: function()
            {
                ...
                var productData = {
                    ...
                    fulfilment_setting: self.fulfilmentSetting(),
            ...
            addConfigurableQuoteItem: function()
            {
                ...
                if(hasSelections) {
                    ...
                    alert('You haven’t selected a product, or the product you selected has been disabled.');
                }
            },
                            ...
                            }},
                        {
                            ...
                            click: function (){
                                ...
                                if(product.has_options && productHasAvailableOptions && self.configureItemDisplayType() != 'custom_options') {
                                    ...
                                    if(product.has_options && productHasAvailableOptions && (product.type_id != 'configurable')) {
                                        self.configurableItemProduct(product);
                                        if(checkSeleteOption()){
                                        ...
                                        if(checkSeleteOption()){
                                            if (customOptionsForm.data('validator')) {
                                                customOptionsForm.data('validator').settings.submitHandler = function(form) {
                                                    addToQuote();
                                                };
                                            }
                                            customOptionsForm.submit();
                                ...
                                function checkSeleteOption(product){
                                    var hasSelections = false;
                                    if(self.productCustomOptionsData()) {
                                        $.each(self.productCustomOptionsData(), function(index, data) {
                                            if(data.value[0].id) {
